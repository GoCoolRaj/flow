...
class LoginBloc extends BaseBloc<LoginEvent, LoginState> {
  ...
  FutureOr<void> _onEmailIdChanged(
    ...
    final normalized = event.emailId.normalizedEmail;
    emit(
      state.copyWith(emailId: normalized, isValidEmailId: normalized.isEmail),
    );
  }
  ...
  FutureOr<void> _onValidateLoginEmailOtpRequested(
    ...
    try {
      ...
      emit(
        state.copyWith(
          status: LoginStatus.userAuthenticated,
    ...
    _otpStateStreamSubscription = _otpBloc.stream.listen((state) {
      switch (state.status) {
        case OtpStatus.resendOtp:
          if ((this.state.emailId?.isEmail ?? false)) {
